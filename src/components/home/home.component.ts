...
export class HomeComponent implements OnInit {
  ...
  protected locationDetails: WritableSignal<{ label: string; value: string }[]> = signal([]);
  protected locationDescription: WritableSignal<string> = signal('Click on a marker to see information about the location.');
  protected mapUrl: WritableSignal<string> = signal('https://www.openstreetmap.org/');
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (this.map && this.L) {
            ...
            this.locationDescription.set('This is your current location. Click on markers to see information about other locations.');

            // Update map URL to point to the user's location
            this.mapUrl.set(`https://www.openstreetmap.org/#map=15/${latitude.toFixed(6)}/${longitude.toFixed(6)}`);
            ...
            userMarker.on('click', () => {
              this.updateLocationInfo('Your Location', 'Current Position', [
                { label: 'Latitude', value: latitude.toFixed(6) },
                { label: 'Longitude', value: longitude.toFixed(6) }
  ...
  private updateLocationInfo(name: string, region: string, details: { label: string, value: string }[], description: string): void {
    ...
    if (latValue && lngValue) {
      this.mapUrl.set(`https://www.openstreetmap.org/#map=15/${latValue}/${lngValue}`);
    }

    // Update website URL based on location type
    const locationType = details.find(d => d.label === 'Type')?.value.toLowerCase() || '';
    switch (locationType) {
      case 'park':
      ...
      case 'library':
        ...
        break;
      case 'market':
        this.websiteUrl.set('https://www.visitbritainshop.com/world/articles/best-markets-in-britain/');
        break;
      default:
